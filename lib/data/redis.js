...
db.prototype = {
	...
	cardHistory: function(room, callback){
		historyKey = REDIS_PREFIX + '-room:' + room + '-cards-history'
		this.getAllCards( room , function (cards) {
			// var historyData = []
			// cards.forEach(card => {
			redisClient.hgetall(historyKey,function(err, res) {
				res == null ? null: callback(res)
			})
			// });
		});
	},

	cardEdit: function(room, id, text, target) {
		redisClient.hget(REDIS_PREFIX + '-room:' + room + '-cards', id, function(err, res) {
			...
			if (card !== null) {
				if(target == 0){
					...
					historyKey = REDIS_PREFIX + '-room:' + room + '-cards-history'
					redisClient.hget(historyKey, today, function(err, res) {
						var history = JSON.parse(res);
						if (history != null) {
							history[card.id] = card.hours;
							redisClient.hset(historyKey, today, JSON.stringify(history));
						}else{
							// no this day history found, we get the latest day
							let breakloop = false
							redisClient.hgetall(historyKey, function(err, res) {
								if(res == null){
									history = {};
									...
									Object.keys(res).sort( (a,b)=>{ return b-a } ).forEach(
										function(key) {
										if(!breakloop){
											var history = JSON.parse(res[key]);
											history[card.id] = card.hours;
											redisClient.hset(historyKey, today, JSON.stringify(history));
											breakloop = true;
										}
	...
	deleteCard: function(room, id) {
		redisClient.hdel(
			...
			// function(err, res) {
			// 	if(res == true){
			// 		redisClient.hdel(REDIS_PREFIX + '-room:' + room + '-cards-history', id);
			// 	}
			// }
			//<<<
		);
